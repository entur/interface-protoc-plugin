...
public final class ProtoTypeMap {
	...
	public static ProtoTypeMap of(@Nonnull Collection<DescriptorProtos.FileDescriptorProto> fileDescriptorProtos) {
		...
		for (final DescriptorProtos.FileDescriptorProto fileDescriptor : fileDescriptorProtos) {
			...
			fileDescriptor.getEnumTypeList()
					.forEach(e -> types.put(protoPackage + "." + e.getName(), DOT_JOINER.join(javaPackage, enclosingClassName, e.getName())));

			// Identify top-level messages, and nested types
			fileDescriptor.getMessageTypeList().forEach(m -> recursivelyAddTypes(types, m, protoPackage, enclosingClassName, javaPackage));
		}

		return new ProtoTypeMap(types.build());
	}

	private static void recursivelyAddTypes(ImmutableMap.Builder<String, String> types, DescriptorProtos.DescriptorProto m, String protoPackage,
			String enclosingClassName, String javaPackage) {
		...
		m.getEnumTypeList()
				.forEach(e -> types.put(protoPackage + "." + m.getName() + "." + e.getName(),
						DOT_JOINER.join(javaPackage, enclosingClassName, m.getName(), e.getName())));

		// Recursively identify any nested types
		m.getNestedTypeList()
				.forEach(n -> recursivelyAddTypes(types, n, protoPackage + "." + m.getName(), DOT_JOINER.join(enclosingClassName, m.getName()), javaPackage));
	}
	...
	/**
	 * Adjust a class name to follow the JavaBean spec. - capitalize the first letter - remove embedded underscores & capitalize the following letter -
